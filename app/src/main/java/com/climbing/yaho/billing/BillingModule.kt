...
class BillingModule(
    ...
    private val consumableSkus = emptySet<Sku>()
    ...
    init {
        billingClient.startConnection(object: BillingClientStateListener {
            ...
            override fun onBillingServiceDisconnected() { // GooglePlay와 연결이 끊어졌을때 재시도하는 로직이 들어갈 수 있음.
    ...
    fun querySkuDetail( type: String = BillingClient.SkuType.SUBS,
                        vararg sku: String,
    ...
    fun purchase( skuDetail: SkuDetails, oldPurchase: Purchase? = null ) {
        ...
        billingClient.queryPurchases(BillingClient.SkuType.INAPP).purchasesList?.let { purchaseList ->
            for (purchase in purchaseList) {
                purchase.skus.forEach {
                    if (it == sku && purchase.isPurchaseConfirmed()) {
    ...
    private fun confirmPurchase(purchase: Purchase) {
        when {
            consumableSkus.contains(purchase.skus) -> {
                ...
                lifeCycleScope.launch(Dispatchers.IO) {
                    ...
                    withContext(Dispatchers.Main) {
                        if (result.responseCode == BillingClient.BillingResponseCode.OK) {
                            callback.onSuccess(purchase)
                        } else { callback.onFailure(result.responseCode) }
                    }
                }
            }
        }
